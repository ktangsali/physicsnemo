...
def parse_speed_from_filename(filename: str) -> float:
    ...
    if m is None:
        raise ValueError(
            f"Cannot parse speed from '{filename}'. Expected pattern: NNN_speedS.vtk"
        )
...
def vtk_to_zarr_one(
    vtk_path: str, zarr_path: str, air_density: float, stream_velocity: float
):
    mesh = pv.read(vtk_path)
    if hasattr(mesh, "cast_to_unstructured_grid"):
        mesh = mesh.cast_to_unstructured_grid().extract_surface()
...
def main():
    ...
    parser.add_argument(
        "--input_dir", required=True, help="Directory with NNN_speedS.vtk files"
    )
    parser.add_argument(
    ...
    vtk_files = sorted([f for f in os.listdir(input_dir) if f.lower().endswith(".vtk")])
    if not vtk_files:
